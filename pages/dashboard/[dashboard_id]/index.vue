...
async function getSlideInformation(slideIndex: number) {
  ...
  if(slideData.url == ""){
    ...
    if(slideData.type == "video"){
      previewUrl.value = slideData.url + "?autoplay=1&mute=1"
    }
    else{
      previewUrl.value = slideData.url
    }
  }
}
getSlideInformation(currentSlideIndex.value)
...
<template lang="pug">
  div.flex.items-center.justify-center.min-h-screen
    div.bg-white.border-4.border-gray-300.rounded-xl.w-full.max-w-2xl.p-16.shadow-2xl
      div.flex.justify-center.space-x-8.text-black-500.text-lg.mb-4(v-if="!editing")
        div
